...
function submitJCL () {
    ...
    jobid=`zowe zos-jobs submit data-set $ds --rff jobid --rft string -u prichar --pw s99arlat --ru false `
    echo $jobid
    echo ''

    echo 'zowe zos-jobs view job-status-by-jobid' $jobid '--rff retcode --rft string'
    retcode=`zowe zos-jobs view job-status-by-jobid $jobid --rff retcode --rft string -u prichar --pw s99arlat --ru false`
    echo $retcode
    echo ''
